...
public class MovieChart extends View {
    ...
    private Map<String, Integer> crunchStats(List<Movie> movieListFromDB) {
        ...
        {
            if (stats.containsKey(currentMovie.getGenre().toString())) {
                Integer value = stats.get(currentMovie.getGenre().toString());
                stats.put(currentMovie.getGenre().toString(), value + 1);
            }
            else
            {
                stats.put(currentMovie.getGenre().toString(), 1);
            }
        }
        return toReturn;
    }
