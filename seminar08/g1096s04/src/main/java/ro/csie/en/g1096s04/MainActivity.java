...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void readJson(View view) {
        ...
        // parse json from the string in raw/movies_json
        try(InputStream inputStream = getResources().openRawResource(R.raw.movies_json)) {
            BufferedReader buffReader = new BufferedReader((new InputStreamReader(inputStream)));
            moviesJsonRaw = buffReader.lines().parallel().collect(Collectors.joining("\n"));
            Log.d(TAG, "Json File: " + moviesJsonRaw);
        }
        catch (IOException ioex)
        {
            ioex.printStackTrace();
        }
        ArrayList<Movie> movies = JSONUtil.getJsonFromString(moviesJsonRaw);
